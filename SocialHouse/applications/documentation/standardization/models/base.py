...
from applications.department.general_data.models import DepartmentInfo, default_department
...
class Document(models.Model):
    ...
    __files_dir__ = 'documentation'
    content_type = ''

    # Model fields
    department_info = models.ForeignKey(to=DepartmentInfo, on_delete=models.DO_NOTHING, related_name="%(class)s",
    ...
    def get_template_file_path(self):
        ...
        template_name = self.__template_name__
        # if callable(template_name):
        #     template_name = template_name()
        cls.__template_path__ = os.path.join(app_folder, 'templates', 'documents', template_name)
        return cls.__template_path__

    # Get uniq filename -> filename: str
    def get_file_name(self):
        # f'{self._meta.verbose_name}_{self.__str__()}.__extension__'
